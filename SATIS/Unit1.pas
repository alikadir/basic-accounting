...
showmessage('Satýþ Yapmak Ýçin Müþteri Tanýmlý Deðil !');
end;
...
if pesinode.Checked=true then durum:= fiyat+' YTL Olarak Peþin Ödendi ';

if  messagedlg('   |=> '+dbedit4.Text+' <=| '+' Firmasýndan Alýnmýþ '+'   |=> '+dbedit5.Text+' <=| '+
    ' Ýsimli Ürünü   '+#13+'|=> '+dbedit1.text+' <=|'+' Ýsimli Müþteriye '+#13+
    'Fiyatý : '+fiyat+' , Ödeme Durumu : '+durum + #13+' Girdiðiniz Bu Bilgiler Doðrultusunda Satmayý Onaylýyomusunuz ?',
...
dbedit15.text:=datetostr(vadetarihi.DateTime);
 ...
 adoquery1.Post;
 msn.Title:='Ürün Satýldý';
 msn.MessageText:=edit2.text+#13+#13+'Ürün Baþarýyla'+#13+'Satýldý..' ;
 msn.Popup;
 adoquery3.Post;
 adoquery2.Post;
 adoquery3.Refresh;
...
showmessage('Stoktaki '+'|=> '+dbedit5.text+' <=| Ýsimli Ürün '+dbedit9.Text+' Adet '+#13+
'Siz '+dbedit9.text+ ' Adet Ürününüzden Fazlasýný Satmak Ýstiyosunuz !!');
end
else
showmessage(' Bölümleri Tam (Eksiksiz) Doldurun ! ');
...
showmessage('Müþterinin '+'|=> '+dbedit11.text+' <=| Bu Ürüne Ait Borcu Görünmüyor !!');
end
else
showmessage('Ödeme Yapmak Ýçin Ürün Tanýmlý Deðil !');
end;
...
odenen:=strtofloat(edit12.text);
 ...
 if  messagedlg('|=> '+dbedit1.text+' <=|'+' Ýsimli Müþteriye '+#13+
    'Satýlmýþ Olan '+'|=> '+edit9.text+' <=|'+' Ýsimli Ürünün Ücretini '+#13+'Ödemeyi Onaylýyomusunuz ?',
     mtwarning,[mbyes,mbno],0)= mryes
 ...
 dbedit16.Text:=edit13.Text;
  ...
  msn.Title:='Ücret Ödendi';
 msn.MessageText:=edit9.text+#13+#13+'Ürünün Ücreti'+#13+'Ödendi ..' ;
 msn.Popup;
...
begin
 ...
 showmessage('Ödenen Parayý Yazmadýnýz !');
 end;
